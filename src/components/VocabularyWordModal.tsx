...
export function VocabularyWordModal({
  item,
  onClose,
}: {
  item: VocabularyWordModalItem;
  onClose: () => void;
}) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`Détails du mot ${word.french}`}
    >
        ...
        <div className="flex items-start justify-between gap-4 border-b border-border p-5 sm:p-6">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full px-2.5 py-1 text-[10px] uppercase tracking-wider ${statusStyle[word.status]}`}>
          ...
          <button
            type="button"
            onClick={onClose}
            className="inline-flex shrink-0 items-center gap-2 rounded-lg border border-border bg-background px-3 py-2 text-sm hover:bg-secondary"
          >
        ...
        <div className="overflow-y-auto p-5 sm:p-6">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl border border-border bg-secondary/30 p-4">
              <div className="text-xs uppercase tracking-wider text-muted-foreground">Occurrences</div>
              <div className="mt-2 font-display text-3xl">×{word.appearances}</div>
            </div>

            <div className="rounded-xl border border-border bg-secondary/30 p-4">
              <div className="text-xs uppercase tracking-wider text-muted-foreground">Première fois</div>
              <div className="mt-2 text-sm font-medium text-foreground">{firstSeenLabel}</div>
            </div>

            <div className="rounded-xl border border-border bg-secondary/30 p-4">
              <div className="text-xs uppercase tracking-wider text-muted-foreground">Sources</div>
              <div className="mt-2 font-display text-3xl">{word.sourceIds.length}</div>
            </div>
          </div>
          ...
          <section className="mt-6">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h3 className="font-display text-lg">Exemples de cours</h3>
            </div>
              ...
              {sourceExamples.map((source) => (
                <article key={source.id} className="rounded-xl border border-border bg-secondary/30 p-4">
                  <div className="mb-2 flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
                    <span>{source.lessonLabel}</span>
                    <span className={`rounded-full px-2 py-0.5 uppercase tracking-wider ${statusStyle[source.status]}`}>
          ...
          <section className="mt-6">
            <div className="mb-3 flex items-center gap-2">
              <BookOpen className="h-4 w-4 text-primary" />
              <h3 className="font-display text-lg">Vu dans les leçons</h3>
            </div>

            <div className="flex flex-wrap gap-2">
              {seenLessonLabels.map((label) => (
                <span key={label} className="rounded-full border border-border bg-secondary/40 px-3 py-1.5 text-sm">
                  {label}
                </span>
              ))}
            </div>
          ...
          <section className="mt-6">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h3 className="font-display text-lg">Erreurs liées</h3>
            </div>

            {relatedMistakes.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border p-4 text-sm text-muted-foreground">
                Aucune erreur liée à ce mot pour le moment.
              </div>
            ) : (
              <div className="grid gap-3 lg:grid-cols-2">
          ...
          <section className="mt-6">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h3 className="font-display text-lg">Grammaire liée</h3>
            </div>

            {relatedGrammar.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border p-4 text-sm text-muted-foreground">
                Aucun point de grammaire lié automatiquement.
              </div>
            ) : (
              <div className="grid gap-3 lg:grid-cols-2">
          ...
          <details className="mt-6 rounded-xl border border-border bg-secondary/20 p-4 text-xs text-muted-foreground">
            ...
            <div className="mt-3 space-y-2">
              <div>
                <span className="font-medium text-foreground">Word ID:</span> {word.id}
              </div>
              <div>
                <span className="font-medium text-foreground">Source IDs:</span> {word.sourceIds.join(", ")}
              </div>
            </div>
          </details>
        </div>
